...
## Training killed due to out of memory (not GPU memory)
First run this command to check the problem: `sudo dmesg -k`, then look for something like `python out of memory`.  
This indicates that the training process was killed due to running out of system memory. I like to monitor  
memory usage with `sudo htop`. The solution is to increase the swap memory size there is a good tutorial about  
it right [here](https://linux.how2shout.com/how-to-increase-swap-space-in-ubuntu-22-04-lts-jammy/#1_Check_current_Swap_Space).
